...
public:
    ...
    RKHSVector() : FuncVector()  { revertToFunc = 0; } 
    RKHSVector(const RKHSVector &src) : FuncVector(src) { revertToFunc = 0; assign(src); } 
    ...
    Vector<SparseVector<gentype> > &x(const char *dummy,                         retVector<SparseVector<gentype> > &tmp) { unsample(); NiceAssert( !revertToFunc ); return xx(dummy,tmp);          }
    SparseVector<gentype>          &x(const char *dummy, int i                                                         ) { unsample(); NiceAssert( !revertToFunc ); return xx(dummy,i);            }
    Vector<SparseVector<gentype> > &x(const char *dummy, const Vector<int> &i,   retVector<SparseVector<gentype> > &tmp) { unsample(); NiceAssert( !revertToFunc ); return xx(dummy,i,tmp);        }
    Vector<SparseVector<gentype> > &x(const char *dummy, int ib, int is, int im, retVector<SparseVector<gentype> > &tmp) { unsample(); NiceAssert( !revertToFunc ); return xx(dummy,ib,is,im,tmp); }
    ...
    virtual void resizeN(int N) { xx.resize(N); alpha.resize(N); return; }
