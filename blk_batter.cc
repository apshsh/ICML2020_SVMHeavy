...
double runfullsim(battSim &simuBatt, const char *datfname, int m, int N, double s, int &ires);
...
public:
    ...
    battSim(double _tdelta, double _theta = DEFAULT_BATTTHETA, double _Q_e = -1, double _SOC = -1)
    {
...
//        V = E_m+(I*R_0)+(I_1*R_1)+(I_m*R_2);
    ...
    void Istep(int tsteps, double I_new, double theta_a_new = -1000)
    {
        int tn = t+tsteps;
    ...
    void Vstep(int tsteps, double V_new, double theta_a_new = -1000)
    {
        int tn = t+tsteps;
    ...
    void Pstep(int tsteps, double P_new, double theta_a_new = -1000)
    {
        int tn = t+tsteps;
...
//errstream() << "phantomx 2: " << I << "\n";
            I.scalarfn_setisscalarfn(0);
//errstream() << "phantomx 3: " << I << "\n";

            for ( t = 0 ; ( t < batttmax() ) && ( simuBatt.V < Vtarget ) ; t += tdelta )
            {
                gentype tt(t/batttmax());
                gentype II = I(tt);
//errstream() << "phantomx 4: " << II << "\n";

                II.finalise(2);
//errstream() << "phantomx 5: " << II << "\n";
                II.finalise(1);
//errstream() << "phantomx 6: " << II << "\n";
                II.finalise();
//errstream() << "phantomx 7: " << II << "\n";

                double Ival = (double) II;
//errstream() << "phantomx 8: " << Ival << "\n";

                Ival = ( Ival < 0.0 ) ? 0.0 : ( ( Ival > Imax ) ? Imax : Ival );
//errstream() << "phantomx 9: " << Ival << "\n";

                simuBatt.Istep(1,Ival);
...
errstream() << "!!!" << t << "!!!";
        ...
        case -1:
        {
            ...
            for ( t = 0, i = 0 ; i < N ; i++ )
            {
                ...
                simuBatt.Istep((int) (tdiff/tdelta),Ival);
        ...
        case -2:
        {
            ...
            resg = runfullsim(simuBatt,datfname,m,N,s,res);
...
int repperiod = 100;
    ...
    for ( i = m ; i < m+N ; i++ )
    {
        ...
        if ( mode[i] == 0 )
        {
            ...
            simuBatt.Istep((int) (tdiff/tdelta),Iin[i],theta_a[i]);
        }
        ...
        else if ( mode[i] == 1 )
        {
            ...
            simuBatt.Pstep((int) (tdiff/tdelta),Iin[i]*Vterm[i],theta_a[i]);
        }
        ...
        else if ( mode[i] == 2 )
        {
            ...
            simuBatt.Vstep((int) (tdiff/tdelta),Vterm[i],theta_a[i]);
        }
        ...
        {
            std::ostringstream tmpbuff;
            tmpbuff << Tset[i] << "\t" << Vterm[i] << "\t" << simuBatt.V << "\t" << Iin[i] << "\t" << simuBatt.I << "\t" << simuBatt.theta << "\t" << mode[i];
            blankPrint(errstream(),n);
            n = nullPrint(errstream(),tmpbuff.str());
            errstream().flush();
        }
