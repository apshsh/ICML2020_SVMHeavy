...
public:
    ...
    template <class S> SparseVector<T> &farassign (const SparseVector<S> &src);
...
template <class T>
template <class S>
SparseVector<T> &SparseVector<T>::farassign(const SparseVector<S> &src)
{
    // No need to worry about shared base here
    ...
    int lstart = src.nearindsize();
    int lend   = src.indsize()-1;

    *indices = (*(src.indices))(lstart,1,lend);
    *content = (*(src.content))(lstart,1,lend);
