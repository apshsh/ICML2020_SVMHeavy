...
const gentype &VVcallbackdef(gentype &res, const gentype &kval, const ML_Base &caller, int iplanr, int jplanr, int iplan, int jplan, int iset, int jset, const gentype &xb, const gentype &yb, int defbasis);
const gentype &VVcallbackdef(gentype &res, const gentype &kval, const ML_Base &caller, int iplanr, int jplanr, int iplan, int jplan, int iset, int jset, const gentype &xb, const gentype &yb, int defbasis)
{
    Vector<int> iokr(2);
    Vector<int> iok(2);
    ...
    return VVcallbackdef(res,2,kval,caller,iokr,iok,iiset,xalt,defbasis);
}
...
double &SVM_Planar::K2(double &res, int i, int j, const gentype **pxyprod, const SparseVector<gentype> *xx, const SparseVector<gentype> *yy, const vecInfo *xxinfo, const vecInfo *yyinfo, int resmode) const
{
    ...
    int inear,ifar,iokr,iok,idiagr,igradOrder,iplanr,iplan,iset  = 0;
    ...
    int jnear,jfar,jokr,jok,jdiagr,jgradOrder,jplanr,jplan,jset  = 0;

    detangle_x(xinear,xifar,xifarfar,xinearinfo,xifarinfo,inear,ifar,ineartup,ifartup,iokr,iok,i,idiagr,&x(i),xxinfo,igradOrder,iplanr,iplan,iset);
    detangle_x(xjnear,xjfar,xjfarfar,xjnearinfo,xjfarinfo,jnear,jfar,jneartup,jfartup,jokr,jok,j,jdiagr,&x(j),yyinfo,jgradOrder,jplanr,jplan,jset);
    ...
    if ( iplanr || jplanr )
    {
        ...
        else
        {
            ...
            gentype gdum('N');
            VVcallbackdef(VVres,kval,*this,iplanr,jplanr,iplan,jplan,iset,jset,x(i).isfarfarfarindpresent(7) ? x(i).fff(7) : gdum,x(j).isfarfarfarindpresent(7) ? x(j).fff(7) : gdum,defProjVV());
